  ...
  void postCommentPerIssue(String project, String repository, String pullRequestId, String sonarQubeURL,
    ...
    for (Object object : issues) {
      ...
      if ((comments != null) &&
        ...
        LOGGER.debug("Issue \"{}\" already added on file {} ({})", issue.getKey(), issue.getPath(), issue.getLine());
      } else {
        
        // check if issue belongs to the Stash diff view
        String type = diffReport.getType(issue.getPath(), issue.getLine());
        if (type == null){
          LOGGER.info("Issue \"{}\" cannot be added to Stash like it does not belong to diff view - {} (line: {})", issue.getKey(), issue.getPath(), issue.getLine());
        } else{
          ...
          LOGGER.debug("Issue \"{}\" has been added ({}) on file {} ({})", issue.getKey(), type, issue.getPath(), line);
          ...
          if (taskSeverities.contains(issue.getSeverity())) {
            ...
            LOGGER.debug("Issue \"{}\" has been linked to a Stash task", comment.getId());
          }
        }
      }
    }
